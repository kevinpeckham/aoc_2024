...
function solveProblem(problem: Problem): number {
	...
	// as well as concatenating the first two integers
	let candidateSolutions = [
		integers[0] + integers[1],
		integers[0] * integers[1],
		Number(`${integers[0]}${integers[1]}`),
	];
	...
	for (let i = 2; i < integers.length; i++) {
		...
		for (const candidate of currentCandidates) {
			...
			// concatenate the solution candidate to the current integer
			const concat = Number(`${candidate}${integers[i]}`);
			if (concat <= solution) {
				newCandidates.push(concat);
			}
		}
...
console.log(solveProblems(data)); // 169122112716571
